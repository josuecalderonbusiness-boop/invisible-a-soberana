...
async function enviarNotifSabado(semNum, tipo) {
  ...
  try {
    const resp = await fetch('https://sendpushnotification-xfy54diwdq-uc.a.run.app', {
      ...
      body: JSON.stringify({ adminKey: 'Sob3rana@Admin2026', nombre: msg.title, mensaje: msg.body, tipo: 'sistema' })
    });
...
// Insertar antes de la función guardarSesion
content = content.replace('async function guardarSesion(semNum)', fn + '\nasync function guardarSesion(semNum)');
