...
// ══════════════════════════════════════════════
// SISTEMA DE ESTRELLAS — Workshop + 7D
// ══════════════════════════════════════════════

const STARS_CONFIG = {
  s0: { posibles: 1, ejercicios: [{ id:'s0_e1', tipo:'textarea', minChars:80 }] },
  s1: { posibles: 1, ejercicios: [{ id:'s1_e1', tipo:'textarea', minChars:150 }] },
  s2: { posibles: 2, ejercicios: [
    { id:'s2_e1', tipo:'auto' },
    { id:'s2_e2', tipo:'textarea', minChars:100 }
  ]},
  s3: { posibles: 3, ejercicios: [
    { id:'s3_e1', tipo:'auto' },
    { id:'s3_e2', tipo:'textarea', minChars:100 },
    { id:'s3_e3', tipo:'auto' }
  ]},
  s4: { posibles: 3, ejercicios: [
    { id:'s4_e1', tipo:'auto' },
    { id:'s4_e2', tipo:'auto' },
    { id:'s4_e3', tipo:'textarea', minChars:100 }
  ]},
  s5: { posibles: 2, ejercicios: [
    { id:'s5_e1', tipo:'multifield', minChars:40 },
    { id:'s5_e3', tipo:'textarea', minChars:150 }
  ]},
  s6: { posibles: 2, ejercicios: [
    { id:'s6_e1', tipo:'textarea', minChars:100 },
    { id:'s6_e2', tipo:'auto' }
  ]},
  s7: { posibles: 3, ejercicios: [
    { id:'s7_e1', tipo:'auto' },
    { id:'s7_e2', tipo:'auto' },
    { id:'s7_e3', tipo:'textarea', minChars:100 }
  ]},
  s8: { posibles: 4, ejercicios: [
    { id:'s8_e1_p1', tipo:'textarea', minChars:80 },
    { id:'s8_e1_p2', tipo:'textarea', minChars:80 },
    { id:'s8_e1_p3', tipo:'textarea', minChars:80 },
    { id:'s8_e1_p4', tipo:'textarea', minChars:80 }
  ]},
  s9: { posibles: 6, ejercicios: [
    { id:'s9_e1', tipo:'auto' },
    { id:'s9_e2', tipo:'auto' },
    { id:'s9_e3', tipo:'textarea', minChars:100 },
    { id:'s9_e4', tipo:'textarea', minChars:80 },
    { id:'s9_e5', tipo:'textarea', minChars:150 },
    { id:'s9_e6', tipo:'multifield', minChars:30 }
  ]}
};
...
function calc7dStars(nodeKey) {
  ...
  const diaKey = parts[parts.length - 1];
  const config = STARS_7D_CONFIG[diaKey];
  if (!config) return { ganadas: 0, posibles: 1 };
  const isDone = state.completed7d && state.completed7d[nodeKey];
  ...
  if (config.tipo === 'preguntas') {
    for (let i = 1; i <= 3; i++) {
      const val = answers[nodeKey + '_p' + i];
      if (val && val.trim().length >= config.minChars) ganadas++;
    }
  } else if (config.tipo === 'diario') {
    for (let i = 1; i <= 4; i++) {
      const val = answers[nodeKey + '_d' + i];
      if (val && val.trim().length >= config.minChars) ganadas++;
    }
  } else if (config.tipo === 'reto') {
    const auto = isDone ? 1 : 0;
    const val = answers[nodeKey + '_reflexion'];
    const escrita = (val && val.trim().length >= config.minChars) ? 1 : 0;
    ganadas = auto + escrita;
  } else if (config.tipo === 'caso') {
    const val = answers[nodeKey + '_caso'];
...
function renderStarsStr(ganadas, posibles) {
  return '★'.repeat(ganadas) + '☆'.repeat(Math.max(0, posibles - ganadas));
}
...
function getTotalStars7D() {
  ...
  return Object.keys(state.completed7d).reduce(function(acc, nodeKey) {
    return acc + calc7dStars(nodeKey).ganadas;
  }, 0);
}
...
"@
$c = $c.Replace("</script>", $motor + "</script>")
$c | Set-Content public/workbook/index.html -Encoding UTF8
Write-Host "Motor agregado"