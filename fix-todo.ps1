...
# Paso 1 - reemplazar estrellas workshop
...
# Paso 2 - reemplazar estrellas 7D
...
# Paso 3 - agregar motor UNA sola vez antes del ultimo </script>
$motorExiste = $c.Contains("STARS_CONFIG")
if (-not $motorExiste) {
    $motor = "
...
function renderStarsStr(ganadas, posibles) {
  var s = ''; for (var i = 0; i < ganadas; i++) s += '" + [char]9733 + "'; for (var i = ganadas; i < posibles; i++) s += '" + [char]9734 + "'; return s;
}
...
Write-Host "Listo - cambios aplicados"